...
static IOReturn sendAndWait(IOHIDDeviceRef dev, uint8_t *pkt) {
    ...
    if (!sGotResp) {
        DDLogWarn(@"LogitechCIDActivator: sendAndWait timed out waiting for response");
...
- (void)handleDeviceAttached: (IOHIDDeviceRef)device {
    ...
    if (openRes != kIOReturnSuccess) {
        DDLogError(@"LogitechCIDActivator: IOHIDDeviceOpen failed with error 0x%x for '%@'", openRes, name);
        return;
    }
