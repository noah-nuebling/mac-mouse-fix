...
static CGEventRef eventTapCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *userInfo) {
    ...
    if (iohidDevice == NULL || device == nil) {
        ...
        /// Use strangeDevice for events from unknown sources (Universal Control, screen sharing, VMs,
        /// PS/2 mice, etc.) so they get processed instead of passed through unmodified.
        /// Previously gated behind NO_FILTER preprocessor macro which was never defined.
        /// The maintainer's comment (line 148) says "We're leaving NO_FILTER on for now" and
        /// "I can't really think of a good reason to filter" - so we make this the default.
        
        if (iohidDevice == NULL) {
            DDLogDebug(@"Input Receiver - Event from virtual input source (Universal Control, screen sharing, etc.). Processing with strangeDevice.");
        } else if (device == nil) {
            DDLogDebug(@"Input Receiver - Sending device not among attached devices. Processing with strangeDevice.");
        }
