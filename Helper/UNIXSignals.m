...
+ (void)load_Manual {
    ...
    ///         -> gnu.org explanation of Termination Signals: https://web.archive.org/web/20240729192135/https://www.gnu.org/software/libc/manual/html_node/Termination-Signals.html
    ...
    ///     - `SIGKILL`: Standard force-termination signal. Sent when force-quitting the agent from Activity Monitor. Probably also sent by launchd at some points. Not catchable at all.
    ///     - `SIGINT`: Sent when pressing CTRL+C while a process is running in the Terminal. Not sure this is relevant for us.
    ///     - `SIGHUP`: Sent to running processes when the controlling terminal closes. Not sure this is relevant for us. For daemons, this signal is sometimes used to make them reload configuration files and stuff. Not sure this is relevant for us.
    ...
    ///
    ///     On **stop signals**
    ///         We're not setting up the handler for stop signals such as `SIGTSTP` or `SIGSTOP` - Those merely temporarily pause the program, instead of terminating it.
    ///
    ///  On debugging
    ///     - When LLDB is attached, standard sigaction() signal handlers are usually not called. Run this in lldb to enable sigaction signal hander to run:
    ///         `process handle SIGTERM --pass true --stop false`
    ///         (We're using "Dispatch Source" now instead of "sigaction()" signal handlers, so this is less relevant, but lldb might still prevent the signals from having their normal effect - creating a core dump and terminating the process, etc.)
    ///
    ...
    for (int i = 0; i < (sizeof signals / sizeof signals[0]); i++) {
        ...
        /// Create the signal-handling Dispatch Source
        dispatch_source_t dispatch_source = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, the_signal, 0, signal_handling_queue);
        
        if (!dispatch_source) {
            
            /// Guard
            ///     Note: Not sure it makes sense to use   perror() here. I don't think the `dispatch` APIs write to `errno`, which perror prints.
...
static void termination_signal_handler(int the_signal) {
    ...
    /// Note: (10.09.2024) Since we're only intercepting simple termination signals, I think we could just call exit(0) here and it might have the same effect? Not sure though. This seems more robust.
    ...
    int ret = sigaction(the_signal, &action, NULL);
    if (ret == -1) {
    ...
    ///     - We could call `exit(0)`  to terminate the process. But that might not behave exactly like the default action, e.g. SIGSEGV creates a 'core image' (debug crash report stuff afaik.) (Update 30.09.2024: Since we're only handling simple termination signals now - SIGTERM, SIGINT, SIGHUP - exit(0) might be enough now.
