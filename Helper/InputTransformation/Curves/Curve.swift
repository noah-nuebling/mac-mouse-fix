...
@objc class Curve: NSObject {
    ...
    @objc func stringTrace(startX x0: Double, endX x1: Double, nOfSamples: Int) -> String {
        ...
        let trace = traceAsPoints(startX: x0, endX: x1, nOfSamples: nOfSamples)
    ...
    @objc func traceSpeed(startX x0: Double, endX x1: Double, nOfSamples: Int) -> [[Double]] {
        ...
        return traceAsPoints(startX: x0, endX: x1, nOfSamples: nOfSamples).map { p in
            let x = p.x
            let y = p.y * x
            return [x, y]
        }
    ...
    @objc func trace(startX x0: Double, endX x1: Double, nOfSamples: Int) -> [[Double]] {
        return traceAsPoints(startX: x0, endX: x1, nOfSamples: nOfSamples).map { p in [p.x, p.y] }
    }
    
    func traceAsPoints(startX x0: Double, endX x1: Double, nOfSamples: Int) -> [Point] {
        ...
        for i in 0..<nOfSamples {
            ...
            let x = Math.scale(value: Double(i), from: Interval(location: 0, length: Double(nOfSamples-1)), to: xInterval)
            let y = evaluate(at: x)
