...
///     How: All non-plist objects in the graph are converted to an NSDictionary holding the internal state, plus the class-name of the object. (Using `kMFPlistCoder_ClassName`)
...
///         Other alternatives:
///             - We could move to non-plist data storage for our object graphs like CoreData or plain NSData archives. However, that's much less human-readable and perhaps more complicated.
///             - We could only use plist types to model our data. But that kinda sucks, and so we introduced MFDataClass to replace NSDictionary in our data-models.
///
...
///             - Debug more easily
///             - Simplify my workflow (E.g. creating a 'default_config.plist' file.)
///             - Perhaps allow different usecases like users editing config files direclty or through a script. (Extremely niche but perhaps nice-to-have for some)
///         Also, theoretically I can reuse this data-modeling and storage system (MFDataClass + MFPlistCoders) in the future for more parts of the app or even other apps. Hopefully the effort will pay off.
///
///     Also see:
