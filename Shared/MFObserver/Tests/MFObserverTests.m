...
#define MF_TEST 0
    ...
    void mfobserver_nsnull_tests(void) {
            
        /// Test result: [Jun 2025]
        ///     - KVO gives us an NSNull instance to signal that the observed property is nil
            
        #define auto __auto_type
        #define mflog(msg...) NSLog(@"MFObserver.m: nulltests" msg)
            
        auto obj = [[NSButton alloc] init];
        
        [obj mf_observe: @"bezelColor" block:^(id  _Nonnull newValue) {
            NSLog(@"new bezelColor class %@", [newValue class]);
        }];
        
        [obj setBezelColor: NSColor.blueColor];
        [obj setBezelColor: nil];
    }
    ...
    #pragma mark - Invoke Tests
    
    @implementation NSObject (MFObserverTests)
        + (void)load {
            mfobserver_nsnull_tests();
        }
    @end
