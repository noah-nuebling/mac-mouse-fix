...
+ (void)sendMessageToHelper:(NSString *)message {
    ...
    BOOL helperRunning = [HelperServices helperIsRunning];
    if (!helperRunning) {
        [NSWorkspace.sharedWorkspace openURL:[NSBundle bundleWithIdentifier:@"com.nuebling.mousefix.helper"].bundleURL];
    }
    if ([message isEqualToString:@"enable"]) {
        helperRunning = YES;
    }
    ...
    // terminate helper if it wasn't running before sending this message
    if (!helperRunning) {
