...
Mac Mouse Fix は、[マウスボタンをキャプチャ](<../../Support/Guides/CapturedButtonsMMF3.md>)するのと同様に、**デフォルトでマウスのスクロールホイールをキャプチャします**。

**スクロールホイールの制御（キャプチャ）を停止するには：**

1.Mac Mouse Fix の'スクロール'タブを開きます
2.**すべて**のオプションをオフにします
   -'速度'オプションについては、'macOS'で設定してください
   -'キーボード修飾キー'の下にあるすべての項目を、各項目の横にある'x'ボタンをクリックして削除します

完了すると、スクロールホイールの制御が解除されたことを知らせるメッセージが表示されます。

**スクロールホイールの制御を*解除する*とどうなりますか？**

Mac Mouse Fixはスクロールホイールの動作を完全に無視するようになります。スクロール中にCPUリソースやバッテリーを消費することはなく、いかなる干渉も行いません。これにより、[MOS](https://mos.caldis.me/)などの他のアプリを使ってスクロールホイールを自由に制御できるようになります。

##関連情報
[キャプチャされたマウスボタン](<../../Support/Guides/CapturedButtonsMMF3.md>)

*(Claude 3.5 Sonnetの多大なる協力により編集されました)*
