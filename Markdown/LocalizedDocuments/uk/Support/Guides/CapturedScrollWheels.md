<!--
THIS FILE IS AUTOMATICALLY GENERATED - EDITS WILL BE OVERRIDDEN
-->
<details>
<summary>󠁧󠁿🇺🇦 Українська</summary>

  [🇬🇧 English](../../../../../Support/Guides/CapturedScrollWheels.md)\
  [🇩🇪 Deutsch](../../../../../Markdown/LocalizedDocuments/de/Support/Guides/CapturedScrollWheels.md)\
  [🇪🇸 Español](../../../../../Markdown/LocalizedDocuments/es/Support/Guides/CapturedScrollWheels.md)\
  [🇳🇴 Norsk bokmål](../../../../../Markdown/LocalizedDocuments/nb/Support/Guides/CapturedScrollWheels.md)\
  [🇧🇷 Português (Brasil)](../../../../../Markdown/LocalizedDocuments/pt-BR/Support/Guides/CapturedScrollWheels.md)\
  [🇻🇳 Tiếng Việt](../../../../../Markdown/LocalizedDocuments/vi/Support/Guides/CapturedScrollWheels.md)\
  [🇹🇷 Türkçe](../../../../../Markdown/LocalizedDocuments/tr/Support/Guides/CapturedScrollWheels.md)\
  [🇨🇿 Čeština](../../../../../Markdown/LocalizedDocuments/cs/Support/Guides/CapturedScrollWheels.md)\
  [🇷🇺 Русский](../../../../../Markdown/LocalizedDocuments/ru/Support/Guides/CapturedScrollWheels.md)\
  **🇺🇦 Українська**\
  [🇨🇳 中文 (简体)](../../../../../Markdown/LocalizedDocuments/zh-Hans/Support/Guides/CapturedScrollWheels.md)\
  [🇯🇵 日本語](../../../../../Markdown/LocalizedDocuments/ja/Support/Guides/CapturedScrollWheels.md)\
  [🌎 Долучайтеся до перекладу :)](https://redirect.macmousefix.com/?locale=uk&target=mmf-localization-contribution)
</details>

# Коліщатко захоплене

Mac Mouse Fix **захоплює коліщатко за замовчуванням**, подібно до [захоплення кнопок миші](<../../Support/Guides/CapturedButtonsMMF3.md>).

**Щоб запобігти захопленню коліщатка:**

1. Перейдіть до вкладки 'Прокрутка' в Mac Mouse Fix
2. Вимкніть **усі** параметри
    - Для параметра 'Швидкість', виберіть 'macOS'
    - Видаліть усі записи 'Клавіш-Модифікаторів' клацнивши кнопку 'x' поруч із кожним модифікатором

Після цього, ви повинні побачити повідомлення, яке натякає на зупинку захоплення коліщатка

**Що відбувається, коли коліщатко прокручування більше не захоплюється?**

Mac Mouse Fix повністю ігноруватиме ваше коліщатко — програма не використовуватиме ресурси процесора чи заряд акумулятора під час прокручування та жодним чином не втручатиметься в роботу. Це дає вам повну свободу використовувати інші програми, як-от [MOS](https://mos.caldis.me/), для керування коліщатком прокручування.

## Також дивіться

- [Кнопки миші захоплені](<../../Support/Guides/CapturedButtonsMMF3.md>)

*(Відредаговано з чудовою допомогою Claude 3.5 Sonnet)*


<br>

<table align="center">
<td>

Надіюсь цей посібник розклав все по поличкам!</td>
<td>

Досі є запитання? Клацніть [тут](https://redirect.macmousefix.com/?locale=uk&target=mmf-support-still-have-questions).</td>
</table>

<!--
    Notes / thoughts: 

    - Should we call it 'capturing' for the scrollwheel? [Sep 2025]
      - Contra: MMF doesn't hide the scroll-events or prevent any default-actions in the same way it does for mouse buttons. 
      - Pro: Capturing is just used like 'Intercepting' by us. The 'hiding from other apps' thing that happens for buttons isn't inherent (But is pretty core to our 'CapturedButtonsMMF3' explation)
    - Should this article even exist? [Sep 2025]
        - - I don't think it's relevant to many users, 
        - + I think it may prevent confusion if we have consistent Capture Toasts for both the 'Capturing sideeffects' on the Buttons and the Scrolling Tab?
        - + The few users for whom it is useful may really appreciate it.
        - - Maybe this should be a footnote at the bottom of CapturedButtonsMMF3.md?
        - - Maintenance overhead. I'm already not including screenshots here and keeping the step-by-step instructions vague to reduce maintenance overhead – and if we do it badly, maybe better not do it at all?
    - Style [Sep 2025]
        - Compared to CapturedButtonsMMF3.md this is **not** broken up into section for maximum scannability. 
        - This is just a long explanatory, text. 
        - Should we break this up into sections like CapturedButtonsMMF3.md for maximum scannability?
            - - More effort
            - - Less laid back / conversational tone (?) (Not sure why I value that. Feels sorta appropriate for this. Since in some way explaining how to use 'competitor' apps might seem like a conflict of interest and sorta weird if I do it in a sterile tone or something? Not sure I'm making any sense.)
            - Easier to parse for readers
        - Should the 'how to uncapture' explanation really be first? [Sep 10 2025]
            - - Most people might come from the uncapturing notification – They'll already know how to uncapture.
            - + The '**What happens when the scroll wheel is *no longer* being captured?**' explanation below feels more natural after the explanation of how to uncapture. (And I think *no longer* is less likely to be missed.)
    - Everything takes me so much time
        - We should just ship this and if I really hate it later I can change it. It won't affect very many people I think.
-->
