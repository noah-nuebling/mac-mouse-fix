...
- 許多內置 Apple 應用程式，如預覽、備忘錄、系統設定、App Store、音樂、TV、圖書和 Freeform
- Adobe Acrobat
- Zotero
- 以及更多！
...
我希望問題現在終於解決了！如果沒有，請在[錯誤報告](http://redirect.macmousefix.com/?target=mmf-feedback-bug-report)或通過[電郵](http://redirect.macmousefix.com/?target=mailto-noah)告訴我。
...
- 修復了 Mac Mouse Fix 有時會重新啟用在系統設定中先前已禁用的鍵盤快捷鍵的問題
- 修復了點擊「啟用授權」時的崩潰
- 修復了在點擊「啟用授權」後立即點擊「取消」時的崩潰（感謝 Ali 的報告！）
...
- 修復了錄製輸入後「+」欄位陰影的視覺故障
- 修復了「輸入您的授權碼」畫面上佔位符文字偶爾會出現偏離中心的故障
- 在「輸入您的授權碼」畫面上禁用了 Touch Bar 文字補全
- 修復了在深色/淺色模式之間切換後應用程式中顯示的某些符號顏色錯誤的問題
