Mac Mouse Fix **3.1.0 Beta 1** přináší skvělý nový překladový systém, na kterém jsem pracoval už delší dobu.\
Jsou tu také některá vylepšení uživatelského rozhraní.

### Nový lokalizační systém!

- Nyní je velmi snadné přispívat kvalitními překlady do Mac Mouse Fix, bez jakýchkoliv technických znalostí.
- K dispozici je spousta okomentovaných screenshotů a poznámek a skvělá vyhledávací funkce, takže překladatelé mohou snadno pochopit, jak přesně se jejich překlady zobrazují v aplikaci.
    - Myslím, že screenshoty a překladatelské rozhraní (aplikace `Xcloc Editor`) jsou nejkvalitnější na trhu a doufám, že ti pomohou vytvářet skvělé překlady!
- Téměř všechny části projektu jsou nyní přeložitelné, včetně 'Průvodce zachycenými tlačítky', GitHub Readme, webových stránek a dalších.
    - Všechny tyto části projektu lze překládat z jednoho centrálního místa se snadným křížovým odkazováním pomocí aplikace `Xcloc Editor`, takže je pro překladatele snadné udržet vše konzistentní.
- Systém je nastaven tak, aby byl velmi snadno udržovatelný, takže jak se Mac Mouse Fix vyvíjí a přibývají nové obrazovky nebo texty, vše se bude aktualizovat automaticky.
- Upozornění: Slovo 'snadné' používám často. Pracovní postupy jsou snadné, ale vymýšlení skvělých překladů je samozřejmě stále spousta tvrdé a obdivuhodné práce! Jen je teď v cestě méně překážek.




Nové překlady již velkorysě poskytli následující lidé:

- **Eduardo Rodrigues**: 🇧🇷 brazilská portugalština
- [@DimitriDR](https://github.com/DimitriDR): 🇫🇷 francouzština
- [@hasanbeder](https://github.com/hasanbeder): 🇹🇷 turečtina
- [Petr Pavlík](http://www.petrpavlik.com): 🇨🇿 čeština

Díky za vaši práci!

Nicméně **všechny jazyky stále potřebují práci**, protože bylo zpřístupněno k překladu mnoho nového obsahu.

Proto:

> [!TIP]
> Podívej se na nový [Průvodce překládáním](https://redirect.macmousefix.com/?target=mmf-localization-contribution), pokud bys chtěl pomoci přinést skvělé překlady uživatelům Mac Mouse Fix po celém světě!

### Další změny

Při používání Mac Mouse Fix v angličtině se toho moc nezměnilo, ale několik věcí bylo aktualizováno v uživatelském rozhraní a hodně se změnilo pod kapotou, aby byly překlady lepší:

- Vylepšený vzhled malých vyskakovacích oznámení.
- Jasnější nápovědy a chybové hlášky na různých místech.
- Klávesa escape nyní může být použita všude k zavření oznámení, dialogů a vyskakovacích oken.
- Na záložku O aplikaci byl přidán odkaz 'Pomoc s překladem'.
- Šířka zobrazení a vyskakovacích oken byla upravena, aby vypadala skvěle v různých jazycích.
- Lepší rozložení textu pro čínštinu a korejštinu, které měly problémy se zalamováním textu v některých malých vyskakovacích oznámeních.
- Nahrazena knihovna pro parsování Markdown, aby se opravily některé chyby formátování v čínštině a korejštině.
- Spousta dalších změn a vylepšení pod kapotou.



---

Předchozí verzi Mac Mouse Fix najdeš zde: [3.0.8](https://github.com/noah-nuebling/mac-mouse-fix/releases/tag/3.0.8)