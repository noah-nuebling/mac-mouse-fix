...
- (void)addAppsToTableWithBundleIDs:(NSArray<NSString *> *)bundleIDs atRow:(NSInteger)row {
    ...
    for (NSString *bundleID in bundleIDsSorted[@"notInTable"]) {
        ...
        for (NSString *columnID in _columnIdentifierToKeyPath) {
            ...
            NSObject *defaultValue = [MainConfigInterface.config objectForCoolKeyPath:keyPath]; // Could use valueForKeyPath as well, because there are no periods in the keys of the keyPath
...
- (void)writeTableViewDataModelToConfig {
    ...
    for (NSMutableDictionary *rowDict in _tableViewDataModel) {
        ...
        for (NSString *columnID in rowDict) {
            ...
            [MainConfigInterface.config setObject:cellValue forCoolKeyPath:overrideKeyPath];
        }
        // Write order key
        ...
        [MainConfigInterface.config setObject:[NSNumber numberWithInt:orderKey] forCoolKeyPath:orderKeyKeyPath];
        orderKey += 1;
    }
    ...
    NSMutableSet *bundleIDsInConfigAndInstalledButNotInTable = [NSMutableSet setWithArray:((NSDictionary *)[MainConfigInterface.config valueForKeyPath:kMFConfigKeyAppOverrides]).allKeys]; // Get all bundle IDs in the config
    ...
    for (NSString *bundleID in bundleIDsInConfigAndInstalledButNotInTable) {
        ...
        [MainConfigInterface.config setObject:nil forCoolKeyPath:overrideKeyPath];
        }
        // Delete orderKey
        NSString *orderKeyKeyPath = [NSString stringWithFormat:@"AppOverrides.%@.meta.scrollOverridePanelTableViewOrderKey", bundleIDEscaped];
        [MainConfigInterface.config setObject:nil forCoolKeyPath:orderKeyKeyPath];
    }
    
    [MainConfigInterface cleanConfig];
    [MainConfigInterface writeConfigToFileAndNotifyHelper];
}

- (void)loadTableViewDataModelFromConfig {
    _tableViewDataModel = [NSMutableArray array];
    NSDictionary *config = MainConfigInterface.config;
    ...
    for (NSString *bundleID in overrides.allKeys) { // Every bundleID corresponds to one app/row
        ...
        if (someNil) { // Only some of the values controlled by the table don't exist in this AppOverride
            ...
            [MainConfigInterface repairConfigWithProblem:kMFConfigProblemIncompleteAppOverride info:@{
                    @"bundleID": bundleID,
