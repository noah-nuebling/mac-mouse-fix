...
- The best possible solution would be custom, not crowdin-based. The most important things we should change is to autogenerate .xcloc files along with autogenerated screenshots produced by XCUI tests. Then have users edit the .xcloc files and send them to me via a github comment or email, instead of having users manually edit .strings files and make pull requests and so on.
